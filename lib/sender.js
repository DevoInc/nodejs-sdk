...
class Sender extends Writable {
   ...
   *  - unref: indicates if the sender should unref the socket
  ...
  constructor(options, callback) {
    ...
    if (options.unref === true) this._socket.unref();
   ...
   * @returns true or the RELP message ID if the stream accepts more events,
   * false or -1 otherwise.
   */
  send(message, callback) {
    const converted = this._convert(message);
    if (this._options.debug) console.log('%s', converted.trim());
    const writeRes = this._socket.write(
      converted, error => this._handleError(error, callback));
    return this._relp.enabled
      ? writeRes ? this._relp.txno : -1
      : writeRes;
  }

  /**
   * Resend an event to the ingestion endpoint (only for RELP).
   *
   * @param {string|Buffer|Object} message payload of the event to send.
   * If an object is passed it will be converted to JSON before sending.
   * @param {Number} txno The RELP message identifier.
   * @param {function(*)} callback Optional function to invoke with (error).
   * @returns The RELP message ID if the stream accepts more events, -1
   * otherwise.
   */
...
commands=syslog`;
  ...
  _emitRsp(txno, body) {
    const command = txno === this._relp.openTxno ? 'open'
      : txno === this._relp.closeTxno ? 'close'
        : 'syslog';
    this.emit('rsp', {txno, command, body});
  }

  _nextRelpTxno() {
    if (this._relp.txno === MAX_RELP_TXNO) this._relp.txno = 0;
  ...
  _transform(message, txno) {
    ...
    const strMessage = typeof message === 'string' ? message
      : Buffer.isBuffer(message) ? String(message)
        : JSON.stringify(message);
    if (strMessage.endsWith('\n')) bits.push(strMessage);
    else bits.push(strMessage + '\n');
