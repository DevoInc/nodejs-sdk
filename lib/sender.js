...
class Sender extends Writable {
  ...
  constructor(options, callback) {
    ...
    this._relp = this._options.relp === true;
    this._txno = 0;
    this._inputBuffer = '';
  ...
  send(message, callback) {
    ...
    return this._relp ? writeRes && this._txno : writeRes;
  }
  ...
  sendClose(callback) {
    ...
    const converted = `${this._nextRelpTxno()} close 3 bye\n`;
    if (this._options.debug) {
      console.log('%s', converted.trim());
    }
    const writeRes = this._socket.write(
...
commands=syslog`;
    const converted =
      `${this._nextRelpTxno()} open ${message.length} ${message}\n`;
    if (this._options.debug) {
      console.log('%s', converted.trim());
    }
  ...
  _onData(data) {
    // pre: this._relp === true
    this._inputBuffer += data.toString();
    this._extractRsps();
  }

  _extractRsps() {
    let m;
    while((m = this._inputBuffer.match(RELP_RSP_REGEX))) {
      const txno = Number(m[1]);
      const length = Number(m[2]);
      const bodyStart = m[0].length + 1;
      if(this._inputBuffer.length < bodyStart + length + 1) {
        // next message is not complete, wait until more data arrives
        break;
      }
      const body = this._inputBuffer.substring(bodyStart, bodyStart+length);
      this._emitRsp(txno, body);
      this._inputBuffer = this._inputBuffer.substring(bodyStart+length+1);
    }
  }

  _emitRsp(txno, body) {
    this.emit('rsp', {txno, body});
  }

  _nextRelpTxno() {
    if (this._txno === 999999999) this._txno = 0;
    return ++this._txno;
  }
