...
class Sender extends Writable {
   ...
   *  - host: the host to send events, optional.
   *  - port: the destination port, optional.
   *  - address: the host and port to send events, optional.
  ...
  constructor(options, callback) {
    ...
    const params = this._getSocketParams(options, callback)
    if (!params) return
  ...
  _getSocketParams(options, callback) {
    const params = {
      ...options,
    }
    if (options.address) {
      const pieces = options.address.split(':')
      if (pieces.length < 2) {
        return this._handleError('Invalid address, should be host:port',
          callback)
      }
      params.host = pieces[0]
      params.port = pieces[1]
    }
    return params
  }
